...
- 旧 writer × 新 runner: 2 行目の boolean 検証に失敗して `exit 2`。旧形式の 2 行目は
  オプション数なので、行数検証まで到達しない

テストで固定できるのは後者だけである。前者は旧 parser を fixture として持たない限り
再現できず、現在は同等の不正形式を新 runner が拒否することを確かめるに留まる。

いずれも **fail-closed**（黙って誤動作せず、起動を拒否する）。バージョンネゴシエーション
は導入しない。「版ずれは起こりえない」のではなく「版ずれ時は安全に拒否し、更新完了まで
起動できないことを許容する」が本設計の立場である。
...
| 引数の安全性 | すべて `#{q:...}` で sh エスケープ済み。**tmux が range 名として解釈し終えた文字列**は、シェルメタ文字を含んでいても単一引数として届く（F8） |
| range 名に書ける文字 | 上の保証は搬送層の話であり、status format 上で任意の文字列を range 名にできるという意味ではない。format パーサが先に解釈するため、`#` は `##` と書く必要があり、空白は style 属性の区切りなので range 自体が成立しない（実測） |
...
18. **回帰**: `mouse on` の状態で、pane 内アプリがモードに応じたイベントを受信する。
    1003 ではクリック・モーション・ドラッグ・ホイール、1000 ではクリックと
    ホイール（1000 はモーションを報告しない追跡モードなので、そこに
    モーションを期待してはならない）
